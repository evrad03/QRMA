...
for cpt=1:1
    ...
    if (jr<10)
        ...
        %fich=['data/mbdalepi-mbdabatang/mbdalepi-mbdabatang20160', num2str(ms),'0',num2str(jr),'.txt']
        %fich=['data/mbdalepi-mbdabatang/mbdalepi-mbdabatang2016050',num2str(cpt),'.txt']
        %fich=['data/mbda-lepi-mbouda/mbda-lepi-mbda20160', num2str(ms),'0',num2str(jr),'.txt']
    else
        ...
        %fich=['data/mbdalepi-mbdabatang/mbdalepi-mbdabatang20160', num2str(ms),num2str(jr),'.txt']
        %fich=['data/mbdalepi-mbdabatang/mbdalepi-mbdabatang201605',num2str(cpt),'.txt']
        %fich=['data/mbda-lepi-mbouda/mbda-lepi-mbda20160', num2str(ms),num2str(jr),'.txt']
    end
    %fich=['data/validation/poli.txt']
    fid=fopen(fich,'rt');
    fgetl(fid);
    fgetl(fid);
    fgetl(fid);
    ...
    while(not(feof(fid)) && i<97)
        ...
        for j=1:8
            ...
            if (j==8)
                ...
                %A(i,10)=A(i,8)-mode(A(:,8));
                %A(i,10)=(((A(i,1)+A(i,2)+A(i,3))-(A(i,4)+A(i,5)+A(i,6)))/3)*D;
                %A(i,10)=(A(i,2)-A(i,5))/D;
            end
        end 
        i=i+1;
    end
    %calcul de la moyenne des atténuations
    %attenuation(cpt,1)=mean(abs(A(:,10)));
    %attenuation(cpt,2)=mean(abs(A(:,11)));
    %attenuation(cpt,3)=mean(abs(A(:,12)));
    ...
    for ri=1:96
        ...
        if(nb>0) %if(A(ri,2)>=20)
            A(ri,11)=A(ri,8)-modes(nb,1);
            %A(ri,10)=round((A(ri,2)-A(ri,2))-modes(nb,1));
            %A(ri,10)=A(ri,8)-mode(A(:,8));
        else
